...
* **Updating keeps what you have collected**: a database written by an earlier release opens as it was — the properties, favourites, notes, tags, price history, monitored searches, and everything you hid or marked sold — with the schema brought up to date around them. That is checked rather than hoped for: a database in the shape release 1.0.0 shipped is part of the test suite, and every build upgrades a copy of it and compares it row by row against what it held before.
