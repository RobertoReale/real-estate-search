...
* **Updating keeps what you have collected**: a database written by an earlier release opens as it was — the properties, favourites, notes, tags, price history, monitored searches, and everything you hid or marked sold — with the schema brought up to date around them. That is checked rather than hoped for: a database in the shape release 1.0.0 shipped is part of the test suite, and every build upgrades a copy of it and compares it row by row against what it held before. **Going back to an older version is not supported**: once an update has changed the database's structure, an older release opening it will say so in the log — one line naming the mismatch and the backups folder — and then carry on, which usually works but is not something to rely on. If you need to go back, either reinstall the newer version, or restore the `case-pre-<version>.db` copy that was taken immediately before the update (see *Backups* below).
* **Backups, and the way back from one**: a copy of `case.db` is written to `backend/backups/` at most once per day (checked at startup; the 14 most recent copies are kept). **Before an update changes the database's structure, a separate copy is taken first** — named `case-pre-<version>.db`, written before the change is applied, and never removed by the daily rotation, so the state you upgraded from stays available however long it takes you to notice something is wrong. The folder is local — point your cloud-sync or a second drive at it if you want off-machine safety. The copies are taken through SQLite's own backup API, so they are consistent even if a scan is writing at that moment.

  **Settings → Data management → Backups** is where you use them, without stopping the app or moving a file by hand. Each copy is listed with its date, its size and the schema version it holds, and you can:
  * **Take a copy now** — before doing anything you are unsure about.
  * **Download** — the copy lands in your downloads folder as a plain SQLite file. Keep it on another disk, or open it with any SQLite tool: nothing here is locked in.
  * **Restore this one** — replaces everything currently in the database with that copy. It asks you to type a word first, and it saves a copy of the *current* state before it does anything, so restoring the wrong file is still recoverable. The dashboard reloads itself afterwards.
  * **Bring one in** — upload a `case.db` carried from another machine. It is checked for being a real database of this app's *before* anything is replaced, and it only joins the list; switching to it is the separate **Restore** step.

  *If you ever copy the database by hand instead, take `case.db-wal` and `case.db-shm` with it*: the newest changes live in those two companion files until the database folds them in, and `case.db` on its own would be missing them. The buttons above are there so you never have to know that.
...
* **Watching a scan happen**: the activity button in the top bar opens the account of what the scanner is doing — which of your searches it is on and on which portal, how many pages it has read, how many listings have come back so far, and which transport it is using when the ladder escalates mid-scan. The pauses between pages are named as pauses, because that is where most of a scan's time goes and a still screen otherwise reads as a crash. **No progress bar is drawn unless the portal itself said how many pages there are** — everywhere else it is a count that rises, since a bar that fills to 90% and stops tells you nothing. Underneath is the journal: one line per search per scan, what it read, what it found and how it ended, still there after the scan finishes and after a reload.
* **In-App Log Viewer**: **Open the log**, under *Diagnostics* on the activity screen, shows the backend's own log — the availability check advancing line by line, DataDome blocks, stack traces — without opening `backend/app.log` in a text editor. It filters by keyword and auto-refreshes every few seconds while open. It is the line-by-line record of what the process did, and the right place to look when the account above does not explain something.
* **Data Management (start fresh)**: **Settings → Data management** has three irreversible resets, each behind a confirmation. *Clear dashboard* deletes all found properties and price history but keeps your search profiles — the next scan rebuilds the grid silently (no notification flood). *Clear price trends* drops only the trend-chart history. *Factory reset* wipes everything back to a fresh install (a backup of the database is saved first). Your notification and login settings are never touched — and each of these is recoverable from the copy taken beforehand, listed under *Backups* in the same panel.
