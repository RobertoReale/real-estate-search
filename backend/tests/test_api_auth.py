...
def _run(path, headers=None, method="GET"):
    return asyncio.run(
        main.require_api_token(_request(path, headers, method), _ok)
    )


def _set_token(monkeypatch, token):
    monkeypatch.setattr(main, "load_settings",
                        lambda: {"api_auth_token": token})
...
def test_correct_bearer_token_passes(monkeypatch):
    _set_token(monkeypatch, "s3cret")
    assert _run("/api/properties",
                {"Authorization": "Bearer s3cret"}) == "PASSED_THROUGH"
