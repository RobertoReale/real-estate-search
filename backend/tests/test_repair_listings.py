...
def test_merge_duplicate_listings_folds_same_url_into_one_property():
    ...
    first, is_new1, _ = upsert_listing(
        db,
        _raw(
            portal_id="111",
            url="https://www.immobiliare.it/annunci/111/",
    ...
    db.commit()
    second, is_new2, _ = upsert_listing(
        db,
        _raw(
            portal_id="222",
            url="https://immobiliare.it/annunci/111?utm_source=email",
            sqm=95,
            rooms=4,
        ),
    )
...
def test_merge_that_revives_a_gone_survivor_clears_gone_at():
    ...
    upsert_listing(
        ...
        _raw(
            portal_id="222",
            url="https://immobiliare.it/annunci/111/",
            sqm=95,
            rooms=4,
        ),
    )
    ...
    survivor.gone_at = datetime.now(UTC)
    db.commit()
