...
def _prop(descriptions: list[str] | None = None, **kwargs) -> Property:
    base = dict(title="", city="", zone="", address="", floor="",
                rooms=None, sqm=None, current_min_price=None)
    base.update(kwargs)
    p = Property(**base)
...
def _prefs(**kwargs) -> dict:
    base = dict(enabled=True, max_price=None, min_rooms=None, min_sqm=None,
                min_floor=None, keywords=[], zones=[])
    base.update(kwargs)
    return base


def test_disabled_scores_none():
    assert compute_match(_prop(current_min_price=100), _prefs(enabled=False,
                         max_price=200)) is None
...
def test_within_budget_is_full_score():
    assert compute_match(_prop(current_min_price=300_000),
                         _prefs(max_price=350_000)) == 100
...
def test_over_budget_decays_linearly():
    ...
    assert compute_match(_prop(current_min_price=385_000),
                         _prefs(max_price=350_000)) == 90
    assert compute_match(_prop(current_min_price=700_000),
                         _prefs(max_price=350_000)) == 0
...
def test_missing_price_scores_zero_against_budget():
    ...
    assert compute_match(_prop(current_min_price=None),
                         _prefs(max_price=350_000)) == 0
...
def test_unparseable_floor_is_dropped_not_zeroed():
    ...
    assert compute_match(_prop(floor="", rooms=3),
                         _prefs(min_floor=2, min_rooms=3)) == 100
...
def test_keyword_fraction_over_title_and_descriptions():
    ...
    prop = _prop(title="Trilocale con balcone",
                 descriptions=["Luminoso, con ascensore e cantina"])
    # 2 of 3 desired features present -> 67
    assert compute_match(prop, _prefs(keywords=["balcone", "ascensore",
                                                "terrazzo"])) == 67
...
def test_zone_preference_matches_city_or_zone():
    assert compute_match(_prop(city="Milano", zone="Isola"),
                         _prefs(zones=["isola"])) == 100
    assert compute_match(_prop(city="Milano", zone="Navigli"),
                         _prefs(zones=["isola"])) == 0
...
def test_annotate_sets_match_score_from_settings():
    ...
    annotate_match_scores(props, {"match_score_enabled": True,
