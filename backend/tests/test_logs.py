...
from app.routers import system as app_system


def test_missing_log_file_returns_an_empty_list(monkeypatch, tmp_path):
    monkeypatch.setattr(app_system, "LOG_PATH", tmp_path / "does-not-exist.log")
    result = app_system.logs_tail()
    assert result["lines"] == []
...
def test_tail_returns_only_the_last_n_lines(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(app_system, "LOG_PATH", log_file)

    result = app_system.logs_tail(lines=3)
