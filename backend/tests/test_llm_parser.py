...
def test_llm_reply_becomes_the_same_search_shape(monkeypatch):
    captured = _enable_llm(
        monkeypatch,
        '{"searches": [{"city": "Milano", "contract": "rent", "max_price": 1200, "min_rooms": 3}]}',
    )
...
def test_missing_city_gets_the_same_warning_as_the_deterministic_parser(monkeypatch):
    ...
    assert any("could not tell which city" in w for w in result["searches"][0]["warnings"])
