...
### 8.10 "A zone's name yields Idealista's zone slug, like Immobiliare" — **False**
The search builder emitted `/vendita-case/milano/bovisa/` for a zone search, by symmetry with Immobiliare. It is a 404 — and so is `/vendita-case/milano-milano/bovisa/`, so the missing province suffix, the obvious suspect, was never the cause. But `/vendita-case/milano/forlanini/` **is live**, 124 listings: Idealista's zone pages exist and the grammar was right all along. They are keyed by slugs of Idealista's own, and a zone's *name* only sometimes happens to be one — 7 of the 8 real searches in the database 404'd, 1 worked. Nothing offline distinguishes them, which is why the failure read as an intermittent portal problem rather than a bug of ours.
...
Three lessons outlived the fix. First, a code comment asserted the grammar and a test asserted the comment, so the suite was green *because* it encoded the bug: an offline test cannot validate a URL grammar, only that we still build the string we decided to build. Second, live verification means watching the portal's own result *total* move when a filter is added (179 → 112 for `trilocali-3`), never the card count — a page holds 30 either way, so counting cards "confirms" any filter, including one the portal ignored. Third, and the reason this entry was rewritten: the first fix generalised "Bovisa 404s" into "Idealista has no zone pages" from a single sample, and shipped that claim into the docs. The profile that scanned `ok` while the theory said it could not was the tell — the anomaly was in the database the whole time, and was noticed only when a `baseline_done` check happened to surface it.

The failure hid two more bugs: on `/cerca/` the contract segment no longer leads the path, so reading `segments[0]` labelled every zone rental a sale, and Immobiliare's `con-ascensore` sits exactly where a zone would, so it parsed as a district named "Con Ascensore".
