...
│   │   │   ├── events.py         # the Server-Sent Events stream the dashboard keeps open
│   │   │   └── system.py         # backend restart, log tail
│   │   ├── livecheck/            # `python -m app.livecheck`: one search, every transport, separately
│   │   │   ├── __main__.py       # the command line and what each flag is allowed to spend
│   │   │   ├── rungs.py          # the ladder of transports, and the capture written for each
│   │   │   ├── budget.py         # requests, spacing, credits, account floor — the refusals
│   │   │   ├── suite.py          # the tracked reference searches: one per shape a search can have
│   │   │   └── report.py         # the table, the saved record, and the redaction before both
│   │   ├── scrapers/
...
│   │       ├── events.py         # one sampler behind the event stream: what changed, once a second
│   │       ├── diagnosis.py      # "try this search": the live-check harness run for one saved search
│   │       ├── demo_data.py      # a deterministic demo corpus, so the dashboard has data without a scan
...
The last two filters — swimming pool and "needs renovation" — closed the loop on the same lesson from the *other* portal. Idealista's tokens (`piscina`, `ristrutturare`) were known for months, but they could not be offered because Immobiliare renders both only as path segments (`/con-piscina/`, `/da-ristrutturare/`) and keeps them there even when a second filter joins them, so no query spelling ever surfaced to read — offering one side alone is the silent asymmetry `idealista_unsupported` exists to prevent, in the direction nothing reports. The query names were finally found not by reading but by **matching result totals**: `?piscina=1` reproduces `/con-piscina/`'s 306, `?stato=5` reproduces `/da-ristrutturare/`'s 1.816, while a plausible `?caratteristiche[]=piscina` answers with the whole city (19.070) — Immobiliare ignores a param it does not know and says nothing (§3's warning shape), which is exactly why the control group is not optional. Only once both portals' spellings were measured did the two filters get exposed.
...
- **Scraping-API transport.** Almost all of the project's most fragile code — the cookie harvester, the Camoufox/Playwright fallback, the block-streak/abort machinery — exists only because scans run from a residential IP DataDome learns. A DataDome-solving scraping API (Scrapfly/ScraperAPI/Zyte) removes that root cause: the target URL is handed to the provider, which returns solved HTML to the *same* parsers. Wrapped at the single `_fetch_once` choke point so nothing else forks; empty key keeps the local path. Relaxes invariants 8/16/18 for the remote path only. The free tier is smaller than it reads — 1,000 credits, and a page with the anti-bot options this app needs cost 25 of them when measured — so the "paid dependency" is real but avoidable, and the app counts what it spends against a monthly ceiling rather than discovering the bill afterwards.
