# A Scan Stopped Returning Listings

[← Back to README](../README.md)

Nobody arrives at this page looking for a vendor. They arrive because a search
that was working came back with nothing, or with a red `Blocked` where the
listings used to be. This page is organised around that: what "nothing" turns
out to mean, how to tell the three causes apart, and what to do about each.

## First, which "nothing" is it?

The activity screen names the outcome of every search it ran, and the three
answers want completely different things from you.

* **`No results`** — the portal was asked and answered "none today". This is not
  a fault: the profile stays healthy and reports 0 listings. (Idealista answers
  such a search with an HTTP 404 — the same code it gives a URL that does not
  exist — so the scan reads the page itself to tell "no flats here today" from
  "no such zone".) If a search you expect to be busy keeps saying this, the
  filters are narrower than you think; open the saved search and check what it
  actually asks for.
* **`Blocked`** — the portal refused the request rather than answering it. Both
  sites sit behind the same anti-bot service (DataDome, which is where the
  `datadome` cookie below gets its name), and a refusal is expected rather than
  a defect: the search shows `Blocked (will retry)` and is retried on the next
  scan, and you are alerted only if it fails several scans in a row (see
  [Scraper health alerts](notifications.md#scraper-health-alerts)). Everything
  below is about this one.
* **`Error`** — the search's URL is wrong, or the portal changed its markup.
  Nothing here will help; open the log (*Diagnostics*, at the bottom of the
  activity screen) and read the failure.

A blocked search means the answer on your screen is **incomplete rather than
empty**, and the app says so where it happens: the journal row carries the
refusal per portal, and the listings it would have returned are simply missing.
That distinction is worth keeping in mind while you work through the rest of
this page — see [`limits.md`](limits.md) for where each such statement appears.
...
Nothing here is required for the app to work — a home connection is trusted most
of the time on its own. These are the levers for when it isn't, from most to
least automatic.
...
Route scraper traffic through a proxy (Settings → Proxy URL). Note a
*datacenter* proxy is refused harder than your home IP; only a residential proxy
helps. You can also list **several proxies** (Settings → *Proxy pool*, one URL
per line): each scraping session sticks to one of them, and an address that gets
blocked is rested for a while so the next attempt leaves through a different IP —
one burned proxy no longer takes every scan down with it.

### Let someone else fetch the pages

Paste a key from a scraping API (Settings → Advanced Scraping → *Scraping API*:
Scrapfly, ScraperAPI or Zyte). Instead of fetching pages from your own IP, each
...
**Idealista's official API** is the only option on this page that is not a
workaround, and the only one that removes the refusal rather than getting past
...
pages. Nothing to refuse, no cookie, no browser. Keys are **not** self-service
...
There is **no equivalent for Immobiliare**: the portal publishes no developer
API, and the third-party "Immobiliare APIs" on offer resell the same scraping
and carry the same refusal. That half of the limit is open, and
[`limits.md`](limits.md) keeps it on the record as open.

---

## What is actually being measured

Everything above is a lever. This part is the map that says *which* lever a given
block calls for. It is written down because the detection changed in 2026 while
nothing here was failing, and because the reasoning behind these levers is not
visible from looking at them.
...
The browser levers work for the same reason in reverse: a cookie earned by a real
...
warning under *Leave from a different address* above, and behind the rule that
automated builds never reach a portal.
