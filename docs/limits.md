...
colouring is reserved for the three that mean *the answer in front of you is not the whole
answer*: a portal that refused the request, a search that stopped at the page cap, and a
search that finished unhindered and still came back with fewer listings than the portal
counted. Twelve caution-coloured lines would train the eye to skip past the one that
matters.
...
| A full sweep can finish and still have read less than the portal counted | `ScanJournalEntryOut.coverage_shortfall` for the verdict, `.listings` and `.total_listings` for the two numbers it is written from | the same row, in the `incomplete` tone — nothing capped this scan and nothing refused it, so every other line on the row reads as complete | `scan.coverageShortfall` |
...
| The paid rung may spend `scrape_api_monthly_credits` credits a month | the field's current value ÷ `SettingsOut.scrape_api_credits_per_page` (published from `transport.ESTIMATED_CREDITS_PER_PAGE`), which turns a credit count into the pages it buys | under the field, in Settings, beside the provider's own remaining balance when the key can read it | `settings.scrapeCredits` |
| With the month's credits spent, searches stop escalating to the paid rung | `ScraperHealthOut.budget` — `spent` against `monthly_credits`, `reached_on`, and `searches` for which ones are affected | the scraper health panel, as a notice for as long as it holds | — (a state notice rather than a `Limit`: it is a condition that ends with the month, not a property of the software) |
...
| `scan.coverageShortfall` | Nothing here can close the gap, because the gap is inside the portal's own answer: its declared total and the listings on its pages are computed at different instants, and a listing published mid-walk pushes every later one down a place — past the last page the walk is allowed. The one lever is a second pass over the same search, which costs a full set of requests again, is exactly the traffic pattern that gets an IP blocked, and still cannot pin a number that moves while it is read. What *is* reachable is already done: the next full sweep re-reads the search, so a one-off shortfall clears itself, and the row says which scan it happened on rather than leaving the counts to be read as the whole. The tolerance below which it is worth saying is measured, not guessed — 247/247 on Immobiliare and 65/65 on Idealista, both reference searches at 1.00 on 2026-09-20, with the margin and its reasoning at `scanner.COVERAGE_TOLERANCE`. | **open** — a count and a listing walk taken at two moments cannot be made to agree at one, and making the app read every search twice would buy a smaller discrepancy at the price of the blocks that produce `scan.portalBlocked`. |
...
| `settings.scrapeCredits` | Raise the ceiling and pay the provider for the credits behind it. The figure it is set against is measured rather than assumed: a Scrapfly page with the anti-bot options this app sends cost 25 credits on 2026-09-10, which is what `ESTIMATED_CREDITS_PER_PAGE` records and what the Settings line multiplies out. [Scrapfly's pricing](https://scrapfly.io/pricing) (checked 2026-09-12) gives 1,000 API credits on signup — 40 pages — and starts paid at $30/month for 200,000. The 900 default is that free grant with a margin, so the app stops before the provider does. | **liftable, but it costs something** — money, at a rate the provider publishes and the app has measured. What it buys is exactly pages: one scheduled hourly scan of a single one-page search is ~600 credits a month, which is the arithmetic the ceiling exists to make visible before it is spent. |
| The paid rung stops when the ceiling is reached | The same money, and nothing else — the stop is the ceiling working. What is deliberately *not* lifted is the silence: a scan that quietly stopped escalating is a scan whose results quietly changed, so the panel names the affected searches and the day it started. One gap in the accounting is known and accepted: the receipts are written with the scan's health row, so a search that failed outright never records what it spent before failing, and the month's total can under-count it. Charging the provider's meter for a scrape that returned nothing would need the spend recorded on the fetching thread, which is the path a failing scan is least able to complete. | **liftable, but it costs something** — the same money. The under-count is bounded by what one failed search can spend in the pages it managed before failing, and it errs towards paying rather than towards refusing to work. |
