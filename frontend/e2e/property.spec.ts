...
test("a property opens, shows its price and its listings, and closes", async ({ page }) => {
  ...
  // "350.000 €", the Italian order — the number is formatted for the language
  // on screen, symbol and grouping together.
  const price = await card.getByText(/^[\d.,]+\s?€$/).innerText();
  ...
  const listings = page.getByRole("heading", { name: /^Annunci trovati \(\d+\)$/ });
  await expect(listings).toBeVisible();
  const found = Number((await listings.innerText()).replace(/\D/g, ""));
  expect(found).toBeGreaterThan(0);
  await expect(page.getByRole("link", { name: /Apri/ })).toHaveCount(found);
...
test("j, k and the arrows walk the results without leaving the property", async ({ page }) => {
  ...
  const position = page.getByText(/^\d+ di \d+$/);
  await expect(position).toHaveText(/^2 di \d+$/);
  const second = at();
  ...
  for (const [forward, back] of [["j", "k"], ["ArrowRight", "ArrowLeft"]] as const) {
    ...
    await expect(position).toHaveText(/^3 di \d+$/);
  ...
  await expect(position).toHaveText(/^1 di \d+$/);
  await page.keyboard.press("k");
  await expect(position).toHaveText(/^1 di \d+$/);
});
